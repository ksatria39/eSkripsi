...
class Progress_proposal extends CI_Controller
{
	...
	public function admin()
	{
		...
		$data['proposals'] = $this->Progress_proposal_model->get_all_proposals();
	...
	public function download_bukti($id)
	{
		...
		if ($data) {
			...
			if (file_exists("file/proposal/bukti/" . $file_path)) {
				$this->load->helper('download');
				force_download("file/proposal/bukti/" . $file_path, file_get_contents("file/proposal/bukti/" . $file_path));
			} else {
				echo "File not found.";
			}
		} else {
	...
	public function mahasiswa1()
	{
		...
		if (count($this->Progress_proposal_model->is_has_accepted_title($user_id)) > 0) {
			...
			$data['judul_list'] = $this->Progress_proposal_model->get_judul($user_id);
	...
	{
		$data = array('data_proposal' => $this->Progress_proposal_model->get_proposal_data_by_mahasiswa($id));
		$data['title'] = "Progress Proposal";
	...
	{
		$data = array('data_proposal' => $this->Progress_proposal_model->get_proposal_data());
		$data['title'] = "Progress Proposal";
		$data['content'] = 'progress/proposal/mahasiswa/mahasiswa';
	...
	public function admin2()
	{
		$data = [
			'title' => "Progress Proposal",
			'content' => 'progress/proposal/admin/admin2',
		];
		$this->load->view('template/overlay/admin', $data);
	}
