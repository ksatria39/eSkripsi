...
class Progress_skripsi extends CI_Controller
{
	...
	public function admin()
	{
		...
		$data['skripsi'] = $this->Progress_skripsi_model->get_all_proposals();
	...
	public function download_bukti($id)
	{
		...
		if ($data) {
			$file_path = $data['bukti']; // Get the image path from database
			$file_name = basename($file_path); // Get the file name
			if (file_exists("file/skripsi/bukti/" . $file_path)) {
				$this->load->helper('download');
				force_download("file/skripsi/bukti/" . $file_path, file_get_contents("file/skripsi/bukti/" . $file_path));
			} else {
				echo "File not found.";
			}
		} else {
	...
	public function mahasiswa1()
	{
		...
		$data['title'] = "Progress Skripsi";
		if (count($this->Progress_skripsi_model->is_has_accepted_title($user_id)) > 0) {
			...
			$data['judul_list'] = $this->Progress_skripsi_model->get_judul($user_id);
	...
	{
		$data = array('data_skripsi' => $this->Progress_skripsi_model->get_skripsi_data_from_dospem_by_mahasiswa($id));
		$data['title'] = "Progress Skripsi";
	...
	{
		$data = array('data_skripsi' => $this->Progress_skripsi_model->get_skripsi_data());
		$data['title'] = "Progress Skripsi";
		$data['content'] = 'progress/skripsi/mahasiswa/mahasiswa';
	...
	public function admin2()
	{
		$data = [
			'title' => "Progress Skripsi",
			'content' => 'progress/skripsi/admin/admin2',
		];
		$this->load->view('template/overlay/admin', $data);
	}
	...
	{
		$data = array('data_skripsi' => $this->Progress_skripsi_model->get_skripsi_data_by_mahasiswa($id));
		$data['title'] = "Progress Skripsi";
