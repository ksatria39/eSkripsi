...
class Score_Skripsi extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->load->model('Skpscore_model');
	}
	...
	public function mahasiswa()
	{
		...
		$ujian = $this->Skpscore_model->getNilai($this->session->userdata('user_id'));
		$data = [
	...
	public function download_nilai($id)
	{
		...
		$data = $this->Skpscore_model->getNilaiData($id);
		...
		$nilaiDospem1 = $this->Skpscore_model->getNilaiDospem1($id, $dospem1->id);
		$nilaiDospem2 = $this->Skpscore_model->getNilaiDospem2($id, $dospem2->id);
		$nilaiDosuji1 = $this->Skpscore_model->getNilaiDosuji1($id, $dosuji1->id);
		$nilaiDosuji2 = $this->Skpscore_model->getNilaiDosuji2($id, $dosuji2->id);
		...
		$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor("template_nilai_skripsi.docx");
		...
		header('Content-Disposition: attachment; filename="nilai_skripsi.docx"');
	...
	public function view_nilai($id)
	{
		...
		$data = [
			'title' => "Nilai Skripsi",
			'content' => 'score/skripsi/view_nilai',
			'dosuji1' => $dosuji1->nama,
			'dosuji1_nidn' => $dosuji1->npm,
			'dosuji2' => $dosuji2->nama,
			'dosuji2_nidn' => $dosuji2->npm,
	...
	public function dosen()
	{
		...
		$dosuji1 = $this->Skpscore_model->getDosuji1($id);
		$dosuji2 = $this->Skpscore_model->getDosuji2($id);
		$dospem1 = $this->Skpscore_model->getDospem1($id);
		$dospem2 = $this->Skpscore_model->getDospem2($id);
		$data = [
	...
	public function nilai_pembimbing($id)
	{
		...
		$ujian = $this->Skpscore_model->getUjian($id);
		$data = [
			'title' => "Penilaian Ujian Skripsi",
	...
	public function insert_nilai_pembimbing()
	{
		...
		$this->Skpscore_model->insertNilai($id, $data);
		$this->session->set_flashdata('success', 'Nilai berhasil disimpan');

		$getNilai = $this->db->where('id', $id)->get('skp_nilai')->row();
		$getRegister = $this->db->where('id', $getNilai->skp_register_id)->get('skp_register')->row();
		$getTitle = $this->db->where('id', $getRegister->title_id)->get('title')->row();

		$nilaiDospem1 = $this->Skpscore_model->getNilaiDospem1($getRegister->id, $getTitle->dospem_1_id);
		$nilaiDospem2 = $this->Skpscore_model->getNilaiDospem2($getRegister->id, $getTitle->dospem_2_id);
		$nilaiDosuji1 = $this->Skpscore_model->getNilaiDosuji1($getRegister->id, $getTitle->dosuji_1_id);
		$nilaiDosuji2 = $this->Skpscore_model->getNilaiDosuji2($getRegister->id, $getTitle->dosuji_2_id);
		...
		$data2 = [
			'status_ujian_skripsi' => $status
		];

		$this->Skpscore_model->insertNilaiTitle($getTitle->id, $data2);
	...
	public function admin()
	{
		...
		$ujian = $this->Skpscore_model->getNilaiAll();
		$data = [
			'title' => "Nilai Ujian Skripsi",
